...
#' vector of weights \code{weights} if it is user-supplied. \itemize{ \item The
#' empirical mean \eqn{P_n effIC(P_n^{k+1})} of the efficient influence curve
#' at \eqn{P_n^{k+1}} scaled by the estimated standard deviation of the
#' efficient influence curve at \eqn{P_n^{k+1}} is smaller, in absolute value,
#' than \code{mic}. \item The total variation (TV) distance between P_n^k and
...
#' \code{superLearningLib} when \code{flavor} is set to "superLearning").  A valid \code{lib} argument must mimick the
#' structure of either \code{learningLib} or \code{superLearningLib} on \code{flavor}.
#'
#' The "superLearning" \code{flavor} requires the \code{SuperLearner} package
#' and, by default, the \code{polspline}, codegam, and \code{ranger}
#' packages.
#'
...
#' corresponds to the continuous exposure variable (e.g. DNA copy number), or
#' "cause" in a causal model, with a reference value \eqn{x_0} equal to 0.
#' \item Column \code{"Y"} corresponds to the outcome variable (e.g. gene
#' expression level), or "effect" in a causal model.  \item All other columns
#' are interpreted as baseline covariates \eqn{W} taken into account in the
...
#' to \code{identity}.
#' @param weights A \code{vector} of weights attached to the observations.
#' Defaults to \code{NULL}, which corresponds to equally weighting all
#' observations. If not \code{NULL}, must be a vector of non-negative numbers,
#' which do not necessarily sum up to 1 (thus allowing to rely, for instance,
#' on the Horvitz-Thompson empirical measure).  The \eqn{i}th entry of the
#' vector is the weight attached to the observation given in the \eqn{i}th row
#' of argument \code{obs}.
...
#' @param nMax An \code{integer} (defaults to \code{30L}; \code{10L} is the
#' smallest authorized value and we recommend a value less than \code{50L} for
...
#' relies on the Super Learning methodology (options "superLearning") or not (option "learning", default value).  In the
#' former case, the \code{SuperLearner} package is loaded.
#' @param lib A \code{list} providing the function \code{tmle.npvi} with the
#' necessary algorithms involved in the estimation of the features of interest.
#' If empty (default) then the default algorithms are used. See \code{Details}.
#' @param nodes An \code{integer}, which indicates how many nodes should be
#' involved in the computation of the TMLE when it relies on the
#' "superLearning" \code{flavor}.  Defaults to
#' \code{1}.  If larger than \code{1}, then the \code{parallel} package is
#' loaded and a cluster with \code{nodes} nodes is created and exploited.
#' @param cvControl The output of a call to \code{SuperLearner::SuperLearner.CV.control()}, or 'NULL' (default value) in which case this is equivalent to providing the output of \code{SuperLearner::SuperLearner.CV.control()}. If \code{flavor}
#' is  set to "superLearning", then   fine-tunes the super learning procedure.
...
#' @param bound A positive \code{numeric} (defaults to \code{1}), upper-bound
#' on the absolute value of the fluctuation parameter.
#' @param B An \code{integer} (defaults to \code{1e5}) indicating the sample
#' size of the data set simulated under each \eqn{P_n^k} to compute an
#' approximated value of \eqn{\Psi(P_n^k)}), the parameter of interest at
#' \eqn{P_n^k}. The larger \code{B}, the more accurate the approximation.
#' @param trueGMu Either \code{NULL} (default value) if the \bold{true}
#' conditional probability \eqn{g(W)=P(X=0|W)} and conditional expectation
#' \eqn{muAux(W)=E_P(X|X\neq 0, W)} are not both known beforehand. Otherwise, a
#' \code{list} with tags \code{g} and \code{muAux} and respective values the
#' two respective \code{function}s (see the related outputs of
#' \code{getSample}).
#' @param iter An \code{integer} (defaults to \code{5}) indicating a maximal
#' number of updates.
...
#' smaller than \code{gmax}.
#' @param gmax A positive \code{numeric} smaller than \code{1}, upper-bound on
#' the range of values of the estimated probabilities \eqn{P_n^k(X=0|W)} that
#' \eqn{X} be equal to its reference value \code{0} given \eqn{W}. Defaults to
#' \code{95e-2}, and must be larger than \code{gmin}.
#' @param mumin A \code{numeric}, lower-bound on the range of values of the
#' estimated conditional expectation \eqn{E_{P_n^k}(X|X\neq 0,W)} of \eqn{X}
#' given \eqn{X\neq 0} and \eqn{W}.  Defaults to the first percentile of
#' \code{X}, and must be smaller than \code{mumax}.
#' @param mumax A \code{numeric}, upper-bound on the range of values of the
#' estimated conditional expectation \eqn{E_{P_n^k}(X|X\neq 0,W)} of \eqn{X}
#' given \eqn{X\neq 0} and \eqn{W}.  Defaults to the 99th percentile of
#' \eqn{X}, and must be larger than \code{mumin}.
#' @param verbose Prescribes the amount of information output by the function.
#' Defaults to \code{FALSE}.
#' @param tabulate A \code{logical}, kept for compatibility, which should be
#' set to \code{TRUE} (its default value). This requires that the joint
#' distribution of \eqn{(X,W)} under \eqn{P_n^k} be a weighted version of the
#' empirical measure (for a faster execution).
#' @param exact A \code{logical}, kept for compatibility, which should be set
#' to \code{TRUE} (its default value).  This requires that the successive
#' updates of the estimators of the relevant features of the true distribution
#' of \eqn{(W,X,Y)} be derived from the current estimators based on exact
#' formulas rather than on their first-order approximations.
#' @param light A \code{logical}, kept for compatibility, which should be set
#' to \code{TRUE} (its default value). This requires that the result of each
#' fit be reduced in size (for a faster execution). Currently implemented only
#' for flavor \code{learning}.
#' @return Returns an object of class "NPVI" summarizing the different steps of
#' the TMLE procedure. The \code{method} \code{getHistory} outputs the
#' "history" of the procedure (see \code{getHistory}).  The object notably
#' includes the following information: \item{obs}{The \code{matrix} of
#' observations used to carry out the TMLE procedure.  Use the \code{method}
...
tmle.npvi. <- function(obs,
    ...
    if (flavor=="superLearning") {
        ...
        if (is.null(cvControl)) {
          ...
          test <- identical(sort(names(cvControl)),
                            sort(names(SuperLearner::SuperLearner.CV.control())))
          if (!test) {
            throw("Argument 'cvControl' is not valid.")
          }
        }
        if (nodes==1) {
