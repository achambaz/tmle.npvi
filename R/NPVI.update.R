setMethodS3("update", "NPVI", function(object,
  ...
  if (cleverCovTheta) {
    ...
    devTheta <- estimateDevTheta(thetaXW, obsT, weights=weights, id=id,
  ...
  if (!useTrueGMu) {
    ...
    gW <- g(extractW(obs))
    devG <- estimateDevG(gW, obsT, weights=weights, id=id,
    ...
    muW <- mu(extractW(obs))
    devMu <- estimateDevMu(muW, obsT, weights=weights, id=id,
