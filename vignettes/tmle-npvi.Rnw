...
\title{Using the \Rpackage{tmle.npvi}  package}
...
If    you    use     the    \Rpackage{tmle.npvi}    package,    please    cite \cite{chambaz12estimation} and \cite{chambaz15targeted}. The statistical method is described in \cite{chambaz12estimation}, and its implementation is described in \cite{chambaz15targeted}. To retrieve the corresponding BibTeX entries, please run:

<<citation, echo=TRUE, eval=FALSE>>=
print(citation("tmle.npvi"), bibtex=TRUE)
@
...
methylation  data of 150 genes from chromosome 17 for  463 breast  cancer samples  from publicly
available TCGA data~\cite{TCGA12}. A genome-wide analysis of the same data set is briefly presented in \cite{chambaz15targeted}.
...
<<testscripts-pvalues, eval=FALSE>>=
...
\bibitem[Chambaz and Neuvial, 2015]{chambaz15targeted}
Chambaz, A. and Neuvial, P. (2015).
\newblock tmle.npvi: targeted, integrative search of associations between DNA copy number and gene expression, accounting for DNA methylation.
\newblock {\em Bioinformatics}, 18:3054-6.
