...
The package includes real data, namely expression, DNA copy number, and DNA methylation data of 150 genes for 463 breast cancer samples from TCGA \cite{TCGA12}.  
...
The data set associated with gene TP53 includes 3 DNA methylation measures across the gene.  
<<pairs>>=
ii <- grep("TP53", nms)
obs <- tcga2012brca[[ii]]
@ 
In this case, there are only \Sexpr{sum(obs[, "X"]==0)} observations with neutral copy number ($X=0$ after shifting).   We arbitrarily set a threshold to $0.02$. This is what the data set looks like, where green points correspond to observations whose copy numbers are smaller than this threshold in absolute value:
<<>>=
thr <- 0.02
...
After thresholding there are  \Sexpr{sum(obs[, "X"]==0)} observations with neutral copy number.  We analyze the data set as follows:
<<>>=
