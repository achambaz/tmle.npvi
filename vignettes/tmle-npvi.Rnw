...
\usepackage{Sweave,amsmath,amssymb,manfnt}
...
<<confInt, fig=TRUE, include=FALSE>>=
hp <- history[, "psi"]
hs <- history[, "sic"]
hs[1] <- NA
...
\begin{figure}
  \centering
  \includegraphics[width=6cm]{fig-confInt}
...
The data set associated with gene TP53 includes 3 DNA methylation measures across the gene.  It can be analyzed as follows:
